...
# Multiple account
RCLONE_NAMES=(gd od cos oss etc)

# Rclone remote folder name (default "")
RCLONE_FOLDER="/webdatas/backups"
...
start_backup() {
    ...
    if ${ENCRYPTFLG}; then
        ...
        openssl enc -aes256 -in "${TARFILE}" -out "${ENC_TARFILE}" -pass pass:"${BACKUPPASS}" -md sha1 -pbkdf2
...
rclone_upload() {
    if ${RCLONE_FLG} && ${RCLONE_COMMAND}; then
        [ -z "${RCLONE_NAMES}" ] && log "Error: RCLONE_NAME can not be empty!" && return 1

        log "Tranferring backup file: ${OUT_FILE} to Google Drive"
        for var in ${RCLONE_NAMES[@]};
        do
            rclone copy ${OUT_FILE} ${var}:${RCLONE_FOLDER} >> ${LOGFILE}
            if [ $? -ne 0 ]; then
                log "Error: Tranferring backup file: ${OUT_FILE} to Google Drive failed"
                return 1
            fi
            echo "${var} upload Success!"
        done
...
delete_gdrive_file() {
    ...
    if ${DELETE_REMOTE_FILE_FLG} && ${RCLONE_COMMAND}; then
        ...
        if [ $? -eq 0 ]; then
            ...
            do
                echo $var
                rclone delete ${var}:${RCLONE_FOLDER}/${FILENAME} >> ${LOGFILE}
                if [ $? -eq 0 ]; then
